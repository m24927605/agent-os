...
- **Cap：審查迭代上限 = 5 回合（founder amendment 2026-06-21，取代原 3）。** 每回合 reviewer 列出問題 →
  **author 必須在該回合修正後**才能進下一回合（不准未修就重審）。**5 回合仍未 PASS → 不卡進度**：依
  [`looping-engineering.md`](./looping-engineering.md) §4 **組織 Staff 等級以上的 coding-agent 專家團隊,由團隊
  決定解法並執行**（非單純停下等人）。環境壞掉/無法重跑指令仍標 `BLOCKED`（fail-closed、不得 merge）。
- **硬性約束（writer↔reviewer 模型獨立）：** 若寫 code 的是 **Claude Code Opus 4.8**,reviewer **必須是獨立的
  Claude Code Opus 4.8**（同模型、fresh context、非作者）。見 §2 的獨立性定義 + `looping-engineering.md` §3。
- 本 review loop 即 loops.elorm.xyz 的 **Independent Verifier Pass**：只信指令輸出、不看作者的理由（見
  `looping-engineering.md` §2）。
