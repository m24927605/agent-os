...
> **已交付（R10-S4，最小子集）**：`kernel/internal/server/checkpoint.go` 的唯讀 `Checkpoint` RPC 在 **與 `Append` 同一把 append mutex** 下原子回傳 `{head_entry_hash, head_sequence, per_source_next_seq}`（proto 新增唯讀 RPC；append-only 表面零改動）。outbox `lastSeq`/`delivered` 的整合三元組擷取留待後續 slice（本 slice 只交付 server 端 head + per-source next 的一致原子讀）。
