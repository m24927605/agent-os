...
- **S2 — TS SDK（author-facing barrel + Fake adapters 匯出）** — **DONE（slice/p2r-r9-s2-ts-sdk-author-barrel）**
  - 新建 `src/sdk/index.ts`：**只** re-export 作者需要的最小公共面：R3 `ToolManifest`/`parseToolManifest`/`ToolSideEffect`、**四個有 `index.ts` barrel 的 Port 介面型別**：`BrainAdapter`（Brain）/`CostGate`/`AgentHosting`/`SandboxAdapter`（ExecutionSubstrate），以及各自的 in-tree Fake adapters（`ScriptedBrain`/`EchoBrain`、`InMemoryCostGate`、`InMemoryAgentHosting`、`FakeSandboxAdapter`）。皆**經各 module `index.ts` barrel**，**非** deep-import `fakes.ts`/`in-memory.ts`/`port.ts`。
