...
| 2026-06-20 | golangci-lint: `can't load config: the Go language version (go1.23) used to build golangci-lint is lower than the targeted Go version (1.25.0)` | `go mod tidy` with `grpc@v1.81` bumped `kernel/go.mod`'s go directive to `1.25.0`; golangci-lint v1.60.3 (built with go1.23) and the dev toolchain (go1.22.4) can't target it | Pin go-1.22-compatible deps: `google.golang.org/grpc v1.64.0` + `google.golang.org/protobuf v1.34.2` (grpc v1.64 = where protoc-gen-go-grpc v1.5's `SupportPackageIsVersion9` codegen lands, and its deps target go ≤1.21); keep `go 1.22` in go.mod (no `toolchain` line); set `GOTOOLCHAIN=local` in `verify:go` so the toolchain never auto-bumps. Generated `*.pb.go` are excluded from golangci-lint. |
