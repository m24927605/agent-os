...
- [x] `pnpm run verify` exit 0
  ...
  exit code: 0
  ```
- [x] dependency-boundary check 綠（`pnpm run deps:check` exit 0；無新跨 module 邊、無 vendor、無 cycle）
  ```
  $ pnpm run deps:check
  ✔ no dependency violations found (69 modules, 148 dependencies cruised)
  exit code: 0
  ```
- [x] low coupling / high cohesion 遵守（resume 只加 fold-before-run，不改 P2-I/FSM/ledger 內部）
  ```
  $ pnpm run deps:check  # depcruise not-to-internal + no-vendor-in-core 維持 exit 0
  ✔ no dependency violations found
  exit code: 0
  ```
- [x] secret-scan 乾淨（effect spy / canary runtime 組裝）
  ```
  $ pnpm run verify  # 末段 secret-scan
  secret-scan: clean
  exit code: 0
  ```
- [x] Docs 更新（design §2.3 crash-idempotency 機制與本 slice 行為一致；見 docs/design/task-agentsession-fsm.md 本 slice diff）
- [x] Adversarial code review = PASS（fresh-context；mutation 必抓：① 把 skip 改成仍呼叫 `runGovernedToolCall`（重複 effect）② 把 stepKey 改成含時間戳/隨機（誤判新 step）③ ledger 錯誤時 fail-open 重跑——三者都應使 §5 測試紅）
  ```
  $ node_modules/.bin/vitest run src/orchestration/task/resume.test.ts
  Test Files  1 passed (1)
       Tests  7 passed (7)
  exit code: 0
  ```
- [x] **Independent Verifier Pass 已執行並 clean**（安全不變量核心：no duplicate external effect、fail-closed、stepKey 決定性；IV 親跑 effect-count spy + 三種 mutation）— Slice R5-S4 passed independent review.
