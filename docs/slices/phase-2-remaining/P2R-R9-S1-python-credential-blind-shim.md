...
- 首次紅燈證據（貼 exit≠0；實作前填）:
  ```
  $ cd python && lint-imports
  ... CONTRACT BROKEN: agentos_shim -> <secret-bearing module> ...
  exit code: <填首次紅燈 exit>
  ```

## (6) Definition of Done（每條附指令證據；實作時填真實 exit code）
- [ ] Test-first 成立（首次 RED 已貼於 §5；git history 可證 doc→red→impl）
- [ ] `pnpm run verify` exit 0（含 `verify:py`：ruff + pyright + pytest + `lint-imports` 全綠）
  ```
  $ pnpm run verify
  ... verify:py: ok ... exit code: <0>
  ```
- [ ] dependency-boundary check 綠（TS `deps:check` 不受影響；Python 由 import-linter forbidden contract 守）
- [ ] low coupling / high cohesion 遵守（`agentos_shim` 單一責任=bundleRef-only 表達；無跨 plane deep import）
- [ ] secret-scan 乾淨（shim/測試無 secret-like 值；canary runtime 組裝、不入 fixture/snapshot）
- [ ] Docs 更新（design/developer-sdk.md §2.2 S1 與本 slice 一致）
- [ ] Adversarial code review = PASS（fresh-context；reviewer mutation：拿掉 forbidden contract 一行 → 違規 fixture 測試應轉綠（證明 contract 真的在守）；恢復後應轉紅）— 摘要: <填>
- [ ]（安全不變量類）Independent Verifier Pass 已執行並 clean（probed：import-linter fail-closed、shim 不洩 secret、verify:py 不可 no-op-green）
