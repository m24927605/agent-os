...
- 首次紅燈證據（待實作時貼 exit≠0）:
  ```
  $ pnpm test src/inference/pdp-adapter.test.ts
  ... FAIL（Cannot find module './pdp-adapter.js'）...
  exit code: <填入>
  ```
...
- [ ] `pnpm run verify` exit 0
  ...
  ... exit code: <填入>
  ```
- [ ] dependency-boundary check 綠（`pnpm run deps:check` exit 0；`inference → policy` 經**本 slice 新建的** `src/policy/index.ts` barrel、inward、無 cycle、`not-to-internal` 不被觸發即無 deep import）
- [ ] low coupling / high cohesion 遵守（不修改 PDP；以注入 `evaluate` 解耦；無 vendor）
- [ ] secret-scan 乾淨
- [ ] Docs 更新（連結回 design §2.2/§4.3）
- [ ] Adversarial code review = PASS（fresh-context；PDP-deny 不可被轉接層翻轉、malformed/throw fail-closed、reason 不洩值皆對抗探測）— 連結/摘要: <填入>
- [ ] （安全不變量類 slice）Independent Verifier Pass 已執行並 clean（PDP 唯一權威、fail-closed、reason non-leak 皆被抓）
