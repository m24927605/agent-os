...
- [ ] `pnpm run deps:check` exit 0（`personal/plan` 只 import `personal/intent` barrel + `src/audit` barrel
      （B0 後）、無 cycle、無 vendor、inward；**不** import 頂層 `src/index.ts`）。
- [ ] low coupling / high cohesion 遵守。
- [ ] secret-scan 乾淨（canary runtime 組裝）。
- [ ] Docs 更新（`src/index.ts` barrel 加 `./personal/plan/index.js`）。
- [ ] Adversarial code review = PASS（fresh-context）— 摘要：<待填>。
- [ ] （安全不變量類 slice）Independent Verifier Pass 已執行並 clean（credential non-leak 出口投影對抗式探測）。
