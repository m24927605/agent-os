...
- [ ] `pnpm run verify` exit 0
  ...
  ... exit code: 0
  ```
- [ ] dependency-boundary check 綠（`pnpm run deps:check` exit 0；runner 只經 orchestration/task + orchestration barrel、無 vendor、無 cycle）
- [ ] low coupling / high cohesion 遵守（runner 無治理決策、無 effect、無 I/O；`runGovernedToolCall`/ledger/clock 皆注入）
- [ ] secret-scan 乾淨（測試 secret canary runtime 組裝）
- [ ] Docs 更新（design §5 已連結；§2.2 compose-over-P2-I 描述與本 slice 一致）
- [ ] Adversarial code review = PASS（fresh-context；mutation：append-before-advance 改成 advance-before-append / denied 後不 break → 須被 §5 測試抓到）— 連結/摘要: <...>
- [ ] （安全不變量類 slice）Independent Verifier Pass 已執行並 clean（短路 0 次後續 effect、append-before-advance fail-closed）
