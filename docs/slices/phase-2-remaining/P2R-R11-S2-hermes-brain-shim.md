...
- [ ] `pnpm run verify` exit 0
  ...
  ... exit code: <填實測，目標 0>
  ```
- [ ] dependency-boundary check 綠（`deps:check` exit 0；hermes import 只在 `runtime/brain/adapters/hermes/`）。
  注意：`no-vendor-in-core` 規則 `from` **不涵蓋 `runtime/`**（`.dependency-cruiser.cjs:60`），brain adapter 的
  vendor 隔離由 (a) 整個 `runtime/` 落規則 `from` 之外 + (b) `not-to-internal` barrel 紀律（adapter 只經 `../../port.js`
  public surface）共同保證；§5 的回歸護欄改打 `src/orchestration/pipeline.ts`（規則涵蓋的 brain 消費者）以命令證明 gate 會攔。
- [ ] low coupling / high cohesion 遵守（shim 僅 import `../../port.js` + `iam/ids` + zod；無 deep import / cyclic / 跨 vendor）
- [ ] secret-scan 乾淨（shim / test 無 secret-like 值；secret canary 為 runtime 組裝、無 source 字面值）
- [ ] Docs 更新（`vendor-adapters.md` §2.2 已述）
- [ ] Adversarial code review = PASS（fresh-context；mutation 驗 credential-blind/fail-closed 測試非 theater；確認 shim 真的無 api_key 通道）— 連結/摘要: <填>
- [ ] （安全不變量類 slice）Independent Verifier Pass 已執行並 clean（adversarially probed：literal secret arg 被 deny+停流、壞 ctx 空流、turnSource-throw 不放行、api_key 無法經 shim 流出）
