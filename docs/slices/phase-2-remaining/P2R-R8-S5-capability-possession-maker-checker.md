...
- [x] `pnpm run verify` exit 0
      ...
      exit code: 0
      ```
- [x] `pnpm run deps:check` exit 0（`src/tenant` 內，僅依賴 `iam/ids`；no-vendor-in-core 綠）
      ```
      $ pnpm run deps:check
      exit code: 0
      ```
- [x] low coupling / high cohesion：純函式 enforce、無跨 module deep import、無 cyclic（deps:check exit 0 佐證）
- [x] secret-scan 乾淨（maker-checker/test 無 secret-like 值；reason 只用 error code/欄位名）
      ```
...
- [x] 切片測試全綠
      ...
      exit code: 0
      ```
- [x] Docs 更新（design §2.6；本 slice）
- [x] Adversarial code review = PASS（fresh-context；嘗試自簽 cap / TOCTOU swap action / 同人雙角 / 跨租 cap 皆 deny）
- [x] **Independent Verifier Pass**（maker≠checker + capability possession + rederive fail-closed + 跨租；mutation：跳過 rederive / 忽略 maker==checker 必被抓）— 本 slice 已通過 independent review
