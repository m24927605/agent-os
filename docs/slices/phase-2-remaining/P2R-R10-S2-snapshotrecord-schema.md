...
- 首次紅燈證據（待填）：
  ...
  ... FAIL (cannot find module './snapshot.js') ...
  exit code: <填實測>
  ```
...
- [ ] `pnpm run verify` exit 0
  ...
  ... exit code: <填實測>
  ```
- [ ] dependency-boundary check 綠（`pnpm run deps:check` exit 0；無 cycle、無 deep import）
- [ ] low coupling / high cohesion 遵守（snapshot.ts 只做 schema + 純轉換；secret detector 注入、不 import audit 內部）
- [ ] secret-scan 乾淨（測試 secret canary 為 runtime 組裝、無 source 字面值）
- [ ] Docs 更新（design 索引已連結）
- [ ] Adversarial code review = PASS（fresh-context；mutation：把 `.strict()` 改 `.passthrough()` → 多餘欄位測試須轉紅）
- [ ] **安全不變量（credential-blind）**：Independent Verifier Pass 已執行——對抗式探測 secret 不入 snapshot、`.strict()` 拒未知欄位、detector 拋例外 → deny-by-default
