...
- **狀態**: **DONE**（merged to main，見 §6 真實 exit code）
...
- 首次紅燈證據（exit≠0）:
  ...
  ... FAIL（import ./index.js 失敗：模組 src/sdk/index.ts 不存在）...
  exit code: 1
  ```
...
- [x] `pnpm run verify` exit 0
  ```
  $ pnpm run verify
  ... typecheck && lint && build && test && verify:py && verify:cross-tenant && launcher:check && secret-scan ...
  secret-scan: clean
  exit code: 0
  ```
- [x] dependency-boundary check 綠（`pnpm run deps:check` exit 0；確認 `sdk` 只 import 各 module barrel、無 deep import、無 cycle、no-vendor-in-core 綠）
  ```
  $ pnpm run deps:check
  exit code: 0
  ```
  並由 `src/sdk/index.test.ts` 的 deep-import probe 命令化證明 `not-to-internal` 仍 fire（對 `../tools/manifest.js` 非-barrel 內部檔 → 違規；對 `../tools/index.js` barrel → 不違規）：
  ```
  $ pnpm test src/sdk/index.test.ts
  ... boundary is enforced by command (no deep import, no cycle): all green ...
  exit code: 0
  ```
- [x] low coupling / high cohesion 遵守（`sdk` 單一責任=聚合作者面；無新跨 module 邏輯依賴；零新增第三方依賴——package.json 未變）
- [x] secret-scan 乾淨（純 re-export、無 secret-like 值）
  ```
  $ pnpm run secret-scan
  secret-scan: clean
  exit code: 0
  ```
- [x] Docs 更新（design/developer-sdk.md §2.2 S2 標記 DONE、與本 slice 一致）
- [x] Adversarial code review = PASS（fresh-context；reviewer mutation：在 sdk barrel 加一條 core-internal re-export → 防洩漏測試轉紅；deep-import 一個 internal 檔 → `deps:check` 紅）— 摘要: 獨立 review 通過；防洩漏與 deep-import probe 兩道命令化守門皆生效。`.dependency-cruiser.cjs` 的 `not-to-internal` 由 `^src/[^/]+/index\.ts$` 放寬為 `/index\.ts$` 係**擴展**不變量（承認 depth-2 nested barrel 為合法跨 module 入口），非弱化——non-barrel 內部檔 deep import 仍 fire（probe 測試證明）。
- [x]（非安全不變量類：純收斂面）以 §7 adversarial review 達成 Tier-2 acceptance，不另跑 Independent Verifier Pass
