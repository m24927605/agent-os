...
- [x] `pnpm run verify` exit 0
  ```
  $ pnpm run verify
  ... typecheck ok · lint: Checked 99 files, no fixes · build ok
  ... Test Files  40 passed | 1 skipped (41) · Tests 397 passed | 1 skipped (398)
  ...   ✓ src/orchestration/task/ledger.test.ts (11 tests)
  ... deps:check ✔ no dependency violations (68 modules, 143 deps)
  ... proto:check ok · openshell:proto:check ok · verify:go ok · verify:py skip · secret-scan: clean
  VERIFY_EXIT=0
  ```
- [x] dependency-boundary check 綠（`pnpm run deps:check` exit 0；ledger 只 import 同 module 的 session(S1)；stepkey 只 import `node:crypto`——**無**跨 module deep import、**無** `audit/*` 邊、無 vendor、無 cycle）
  ```
  $ pnpm run deps:check
  ✔ no dependency violations found (68 modules, 143 dependencies cruised)
  DEPS_EXIT=0
  ```
- [x] low coupling / high cohesion 遵守（port 與 fake 同檔但公共面僅 port+fake+harness；append-only 無 delete/update 公共面；no-vendor-in-core 真實 src 樹無違規 — 見 build/no-vendor-in-core.test.ts 綠）
- [x] secret-scan 乾淨（hash fixture 為 runtime 組裝、無 source 字面 secret；`secret-scan: clean`）
- [x] Docs 更新（本 §5/§6 已填真實 exit code；狀態=DONE）
- [x] Adversarial code review = PASS（fresh-context；mutation：把 append 改成可覆寫 / 把 stepKey 改成含時間戳 → 被 §5 append-only / 決定性 stepKey 測試抓到）— 摘要: 獨立 review 已通過（slice R5-S2 passed independent review）
- [x] （安全不變量類 slice）Independent Verifier Pass 已執行並 clean（append-only 不可改寫、stepKey 決定性 = 不重複 effect 的根；11 ledger 測試含 contract harness 全綠）
