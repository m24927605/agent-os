...
- [x] `pnpm run verify` exit 0
  ```
  $ pnpm run verify
  ... Test Files 61 passed | 1 skipped (62) / Tests 590 passed | 1 skipped (591)
  ... deps:check ✔ no dependency violations found (101 modules, 243 dependencies cruised)
  ... secret-scan: clean
  exit code: 0
  ```
- [x] **authoring 驗收**：`agentos manifest lint src/sdk/templates/tool-manifest.example.json` exit 0；對違規 fixture exit 1（貼兩個 exit code）
  ```
  $ node dist/cli/main.js manifest lint src/sdk/templates/tool-manifest.example.json
  ok github-create-issue@1.0.0
  exit code: 0
  $ node dist/cli/main.js manifest lint <violating: destructive + requiresApproval:false>
  invalid: [ { "code": "custom", "path": ["requiresApproval"],
              "message": "sideEffect \"destructive\" implies requiresApproval: true" } ]
  exit code: 1
  ```
- [x] dependency-boundary check 綠（`pnpm run deps:check` exit 0；`templates` 只經 SDK barrel、無 deep import、no-vendor-in-core 綠）
  ```
  $ pnpm run deps:check
  ✔ no dependency violations found (101 modules, 243 dependencies cruised)
  exit code: 0
  ```
- [x] low coupling / high cohesion 遵守（`templates` 單一責任=合法範本；無 schema 邏輯——`index.ts` 僅經 `../index.js` SDK barrel 消費 R3 `parseToolManifest`）
- [x] secret-scan 乾淨（範本/fixture 無 secret-like 值；bundleRef 只是參照字串；`secret-scan: clean`，且 `templates.test.ts` 正向斷言無 secret-shaped 值）
- [x] Docs 更新（design/developer-sdk.md §2.2 S4 標記 DONE；新增 `docs/sdk/tool-manifest-authoring.md`）
- [x] Adversarial code review = PASS（fresh-context；reviewer mutation：把範本改成違規（destructive+免審批）→ `parseToolManifest`/`manifest lint` 轉紅，證明範本受 R3 護欄約束）— 摘要: 範本與 on-disk JSON byte-equivalent（測試 pin，無 drift）；違規 fixture 經 R3 guardrail B 被擋（exit 1 已驗）；無新依賴、無 schema 邏輯、無 deep import。
- [x]（非安全不變量類：作者體驗 + 範本）以 §7 adversarial review 達成 Tier-2 acceptance，不另跑 Independent Verifier Pass
