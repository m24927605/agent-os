...
- [x] Test-first 成立（首次 RED 已貼於 §5：`pnpm test src/audit/ingest/client.test.ts` → `FAIL (cannot find ./client.js)`、exit 1）
- [x] `pnpm run verify` exit 0
  ```
  $ pnpm run verify
  ... typecheck / lint (54 files) / build / test (20 files, 147 tests passed) ...
  ... deps:check ✔ / proto:check ok / verify:go ok / verify:py skip / secret-scan clean ...
  exit code: 0
  ```
- [x] dependency-boundary check 綠（`pnpm run deps:check` exit 0；commitgate 經 barrel、無 cycle、無 vendor）
  ```
  $ pnpm run deps:check
  ✔ no dependency violations found (37 modules, 78 dependencies cruised)
  exit code: 0
  ```
- [x] low coupling / high cohesion 遵守（client.ts 僅經 `../../commitgate/index.js` barrel 取型別；其餘為 audit 模組內 import；注入 transport、無自身外部副作用）
- [x] secret-scan 乾淨（canary 於 runtime 組裝 `sk-${"z".repeat(32)}`，不入 source/fixture/snapshot）
  ```
  $ pnpm run secret-scan
  secret-scan: clean
  exit code: 0
  ```
- [x] Docs 更新（本 slice doc 狀態 DONE + §6 證據；barrel `src/audit/ingest/index.ts` 匯出 `createIngestClient`）
- [x] Adversarial code review = PASS（mutation：transport reject 後仍推進 counter → sequence 測試轉紅；移除 canonicalize 直送原始 event → canary 測試轉紅）
- [x] （安全不變量類 slice）Independent Verifier Pass 已執行並 clean（probe：reject 路徑零 counter 推進、bytes 零 canary）
  ```
  $ pnpm test src/audit/ingest/client.test.ts
  ✓ src/audit/ingest/client.test.ts (5 tests)
