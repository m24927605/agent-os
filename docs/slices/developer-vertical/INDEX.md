...
| **DV2** | 護城河真證明:暴露 kit pubkey + gated e2e(build 真 `kernel/cmd/verifier` + **checksum 對 SHA-256SUMS** → kit 的 ed25519-signed 鏈 `verifyEvidenceChain` → **intact〔跨語言 byte-match〕/ tampered→broken / wrong-pubkey→非零**)。**驗 live kernel 鏈撞 kernel-UNSIGNED(P2R-PV-S3a)→ 護城河證明改用 kit 自簽鏈,驗 kernel 鏈留後續** | DRAFT(依 DV1)|
