...
| **DV4** | Python plane 端到端 demo + 邊界證明:committed bundleRef-only proposal fixture(byte-equivalent 契約)+ Python 測試(shim 產它)+ TS 測試(讀 fixture → governed 管線 accepted + WORM)+ 雙重 credential-blind(Python ValueError / TS denied@screen)+ import-linter。**fixture/JSON 邊界,非 live runtime 整合(R11)** | DRAFT(spec 好,待開工)|
