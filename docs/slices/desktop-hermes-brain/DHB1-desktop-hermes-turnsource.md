...
- **DHB1(in-repo 可達成)**:`DesktopHermesTransport` port(API-shape-agnostic)+ `DesktopHermesTurnSource implements HermesTurnSource`(raw→`HermesTurn`)+ **Fake transport** 證接縫 + contract test。
- **DHB1 誠實不宣稱**:**未**綁真 desktop Hermes endpoint(其 local API 未知,需 DHB2 從 app/docs 確認)、**未**證 propose-only 模式 live。DHB1 宣稱的是 **① 接縫到位(Fake transport 下 turn→BrainEvent→治理全鏈綠)② credential-blind 結構性保持 ③ fail-closed**。

## (1) ID + Title
SLICE-DHB1 —（a)`DesktopHermesTransport` port:`submit(intent: string): AsyncIterable<RawHermesFrame>`(把 intent 交給本機 Hermes、串回原始 turn 幀;**只送 intent 文字 + AgentContext-derived 識別,絕不送 Agent OS secret**);(b)`parseFrame(raw): HermesTurn`(只取 planText/toolCalls/memoryOps/skillOps;**忽略任何 api_key/credential 欄位**,malformed → 略過該幀 fail-closed);(c)`DesktopHermesTurnSource implements HermesTurnSource`(包 transport + parser,`turns(intent)` 串 `HermesTurn`);(d)`FakeDesktopHermesTransport`(in-tree,腳本化原始幀,含「夾帶 api_key/字面 secret」的對抗幀)證接縫;(e)contract test + gated live e2e 骨架 `e2e:live-desktop-hermes`(缺 `AGENTOS_DESKTOP_HERMES_ENDPOINT`/app → clean BLOCK,exit 0)。
...
- In-scope:
  - `DesktopHermesTransport` port + `RawHermesFrame`(寬鬆 raw 形狀;parser 只取已知欄位)。
  ...
  - `parseFrame`:raw→`HermesTurn`,**只讀 planText/toolCalls/memoryOps/skillOps**;任何 credential/api_key 欄位**永不讀入**;malformed 幀略過(fail-closed,不中毒整串)。
  - `FakeDesktopHermesTransport`(in-tree,腳本化 + 對抗幀:夾帶 api_key、字面 secret、malformed)。
  ...
  - gated live e2e 骨架:`scripts/e2e-live-desktop-hermes.sh` + `e2e:live-desktop-hermes`(gate on `AGENTOS_DESKTOP_HERMES_ENDPOINT`;缺 → `BLOCKED` exit 0;**不在 `pnpm run verify` 內**)。
...
- Out-of-scope（誠實標記,= DHB2/live）:
  - **綁真 desktop Hermes local gateway endpoint/protocol**(HTTP/WS、路徑、auth、propose-only 模式確認)= DHB2,需你本機跑 app + 確認其 API。
  - **propose-only 行為的 live 證明**(desktop Hermes 只回提案、不自行執行)= DHB2。
...
## (4) Design delta + 依賴方向
- 新 port + 1 真 impl + 1 Fake + parser;**`HermesTurnSource` 介面不變**(`DesktopHermesTurnSource` 實作它)。`HermesBrainShim`/screen/pipeline 不動。無新依賴(transport 真實作的 fetch/WS 留 DHB2;DHB1 用 Fake)。
