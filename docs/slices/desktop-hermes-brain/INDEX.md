...
| **DHB3b**(live,**user-initiated**) | 綁真 `hermes acp 0.17.0` 的**回饋邊 dialect**(同 sessionId 重 prompt tool-result 是否續推 / 沿用脈絡 / propose-only enum);dialect 分歧 → fail-closed 診斷,絕不退化讓 Hermes 自跑。**用你 credits → 由你親跑** | ✅ **DONE + LIVE 通過(2026-06-24)**:gated 多輪 live test(skip-under-verify)+ **真 `hermes acp` 閉環綠**(turns=2、governedTurns=2、effectRuns=2、stoppedBy=terminal;**✓ RETURN EDGE CONTINUED → Design B 真 dialect 成立**;propose-only cwd `[]`、credential-blind);獨立 Opus4.8 review(harness)+ live 親驗|
