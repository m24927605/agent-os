...
- **狀態**: **DHB2a DONE**（merged;writer=Backend Architect/Opus4.8;獨立 Opus4.8 reviewer=PASS;propose-only/fail-closed/credential-blind〔wire+child-env〕/fs-terminal-deny 經 mutation 證實;真子進程 + early-break 清理無 orphan;DHB1/seam 未動;採納 reviewer M1+M2）。**DHB2b OPEN**(live,user-initiated)。
