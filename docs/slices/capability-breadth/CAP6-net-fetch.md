...
- **⬆️ SUPERSEDED by Option D（2026-06-30, on main）**:argv 由 `--noproxy *`（proxy-DISABLED,OpenShell 下無法外連)改為 **proxy-PINNED** `curl -q --globoff -x <AGENTOS_OPENSHELL_EGRESS_PROXY> -sS -- {url}`(預設 `http://10.200.0.1:3128`,無效值 fail-closed)。net.fetch 現可外連到 **PDP allowlist + OpenShell policy 雙重授權**的 host（live-proven HTTP 200,見 `net-fetch-egress.live.test.ts` + `deploy/personal/netfetch-egress-policy.example.yaml`)。git.push 同樣 pin(`-c http.proxy=`)。且**上方「exec.run 任意網路命令 NOT 被 PDP egress fold gate」已修正**:exec.run 的 proxy-redirect（glued `-xhost` / bare `-x host` / `env *_PROXY=`)現在會把 proxy host 投進 networkHosts → egress fold deny(in-tool guard,substrate seal 之外的 defense-in-depth)。**仍 open**:每沙箱自動套 egress policy(生產接線,設計探勘中)。
