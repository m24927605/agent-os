...
- [ ] RED → verify exit 0(net.fetch manifest/binding/register + toEnv placeholder;**egress fold 真 gate net.fetch**〔allowlist host 過、非-allowlist denied@policy、default deny-all〕;credential placeholder〔拒 literal、發 placeholder〕;strict/no-shell;commit-before-effect;**byte-identical**〔14→15〕;mutation 證;depcruise no-vendor-in-core 綠;secret-scan clean;無新依賴);獨立 Opus 4.8 review PASS。
