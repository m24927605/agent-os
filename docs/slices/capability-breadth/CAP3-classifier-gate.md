...
- [x] **DONE（merged)**:`ToolManifest` + required `containment`(`in-sandbox|network-egress|host-fs-write`)+ 新 `src/tools/capability-containment.ts`(`requiredPrimitives` 表 + `WIRED_PRIMITIVES`=空 + `assertRegisterable`)+ ToolRegistry constructor & register() 掛 gate(原子化:constructor 用 local staged map,throw-before-insert)。更新 16 個 manifest literal(14 exec/git + fsReadManifest + CLI/SDK/三面/dev-kit fixtures + SDK example JSON)為 `containment:"in-sandbox"`(schema 9→10 欄位)。RED → verify **exit 0**(1338 passed + 29 skipped;classifier+gate 測;**skip-gate mutation 翻 7 gate 測**;WIRED 注入可註冊〔gated-on-wired 非永拒〕;**14 工具全過、advertised 14、零行為改變**;PDP runtime 不變〔authorize.ts 零引用〕;containment fail-closed〔缺/未知→parse 拒〕;depcruise no-vendor-in-core 綠+bite;secret-scan clean;無新依賴)。獨立 Opus4.8 review PASS:gate refuse 非 vacuous + **無繞過路徑**(private #byName、原子建構)+ gated-on-wired + 零行為改變 + PDP runtime 不變 + byte-identical。1 nit(approval dedup guard 防禦性,無害)。
- **spine 化為可執行的閘**:Slice 4/5/6/9 必須先接其 primitive(加入 WIRED)+ 設對 containment,工具才能註冊——結構上不能早於 primitive。
