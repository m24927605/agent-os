...
- [ ] RED → verify exit 0(registry wired param + budget/maker-checker approver + bin closure requiresApproval + bin 注入 budget approver + wired "approval";合成 destructive 工具端到端〔pre-auth→approved→effect;非→denied@approval→effect 0〕;unconfigured deny-all;**14 工具 byte-identical**;mutation 證;depcruise/secret-scan clean;無新依賴);獨立 Opus 4.8 review PASS。
