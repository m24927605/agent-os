...
- [x] **DONE（merged)**:5 git tool(status/diff/log read+idempotent、add/commit write)manifest+binding+registration+projector,FIXED argvPrefix、string-only args(既有 converter,**無擴充**);**+ additive `allow-git`(`git.**`)allow rule in bin**(RED 揪出:registration≠authorization,原只有 `exec.**` → git 工具 advertised-but-denied;OR-combined,`exec.**` byte-identical)。RED → verify **exit 0**(1311 passed + 29 skipped;22 git 測;split-message/drop-`--` mutation 翻 no-shell、remove-rule 翻所有 8 git 工具〔load-bearing〕;converter/closed-loop/barrel byte-unchanged;depcruise-bite;secret-scan clean;無新依賴)。獨立 Opus4.8 review PASS:fixed-subcommand no-shell 非 vacuous、**`git.**` 正確 scoped 不過度授權**(registry deny-by-default pre-screen 在 allow rule 前擋未註冊名 → **git.push 未註冊仍 denied**;`^git\..*$` 不收非-git;tenant-scoped)、strict deny、credential-blind、commit-before-effect、byte-identical(9→14)、requiresApproval:false 無 approval gate。1 MINOR(git.log 硬上限 -n50、diff/status 無 pathspec)= 合理 bounded posture,非缺陷。
- **能力面 9 → 14 工具。** `git.push` 延後(network/destructive 邊 → Slice 5/6);git binary + repo 是 sandbox/deploy fact;fake-proven,LIVE 需真 sandbox。
