...
- [x] **DONE（merged)**:`AuthorizeDecision.external?`/`projection?` + bin closure 從 containment 設 + pipeline post-effect boundary append(`action:"effect.boundary-crossed"`)。**只 external+executed 出 boundary**〔denied/aborted/in-sandbox→0,永不 boundary-without-effect〕;intent+boundary 配對;audit-only PDP-sovereign;boundary-append 失敗 surface(`boundary:"append-failed"`,executed 不變);**byte-identical**(in-sandbox 14 工具不變)。RED → verify **exit 0**(1448 passed + 29 skipped;3 non-vacuity mutation〔boundary-when-not-external/boundary-on-denied/closure-not-setting-external〕翻;depcruise/secret-scan clean;無新依賴)。
- **⚠️ Round-1 review FAIL [MAJOR]→ 已修→ re-review PASS**:首版把**完整 projection(含 `argvRedacted`)**寫進 WORM `boundaryProjection`,違反 R9b-1「projection NEVER a WORM/audit sink」(best-effort redact,非標準 secret 殘留);**今天就可觸發**——`isAllowedFetchUrl` 不驗 path/query,net.fetch 的 `?access_token=…` 整個 URL 進 argvRedacted→WORM。**修法**:改 `boundarySummary`(嚴格 **allow-list** `boundarySummaryFromProjection`:只 networkHosts〔userinfo-stripped/host-only〕/operationClass/usesShellInterpreter/truncated/destructiveFlags/version;**drop argv0/argvRedacted/argc**)。新增 **URL-query canary 端到端測**(RED:canary 原本進 WORM;GREEN:修後不進任何 append byte;reviewer 親自 re-probe 真 net.fetch 路徑確認)。intent event 本就 neutral(非洩漏)。獨立 Opus4.8 re-review **PASS**(allow-list 無未來欄位洩漏、canary 端到端不進 WORM、新測非 vacuous、audit 價值〔networkHosts+operationClass〕保留、無回歸)。
