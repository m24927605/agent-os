...
- [x] **IT1b DONE（merged)**:`src/runtime/spendguard/config-root.ts`(vendor 區,非核心三面)的 `integrationsFromEnv(env, extra?)` + `costGateForFromEnv(env)`(Enterprise per-tenant)。讀 `SPENDGUARD_UDS_PATH`(+ `SPENDGUARD_BUDGET_ID`/`SPENDGUARD_UNIT_ID`/`SPENDGUARD_WINDOW_INSTANCE_ID`〔writer 發現 budgetClaim 第三欄必填,加為 fail-closed key,reviewer 判 sound〕/ optional `SPENDGUARD_TENANT_ASSERTION`)→ `SpendGuardCostGate(createDecisionLedgerTransport(...))`(grpc-js lazy,構造不連線);UDS 未設→無 costGate(三面 fallback InMemory)。RED → verify **exit 0**(1118 passed + 26 skipped;config-root 17 測;**fail-closed**:UDS 設但 topology 缺/空→**startup throw**〔絕不靜默 fallback;silent-fallback mutation 翻 6 測〕;absent-vs-partial 區分正確;credential-blind〔只非-secret id、error 只印 key 名〕;per-tenant 獨立〔memoize mutation 翻〕;secondaries 透傳〔AGT 注入點〕)。**no-vendor-in-core 綠**(config root 在 vendor 區 import vendor 合法;boundary 對真核心檔仍 bite);surfaces/IT1a/adapter/decision-transport byte-unchanged。獨立 Opus4.8 review PASS(零 findings)。
- [x] **文件 DONE**:README「Enabling SpendGuard / AGT (config-driven)」段——SpendGuard 設 `SPENDGUARD_*` env + `integrationsFromEnv()`/`costGateForFromEnv()` 傳入三面;AGT = in-repo `AgtSecondaryPolicy` 包**你的** AGT engine(evaluate seam)當 advisory secondary 傳入。誠實標明:SpendGuard 需外部 sidecar + topology + release 未接;AGT 引擎自備;注入只能更嚴、PDP sovereign;fail-closed。
