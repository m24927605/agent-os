...
| **HDI2**(有用的 governed 工具集 — capability 半) | 超越 exec.echo/ls,長出 bounded governed 工具 bindings(每個 = manifest + `ExecToolBinding{argvPrefix,strict argSchema,toArgv}` 永不 sh -c + allow rule + 測)。先 read-only 安全集(exec.cat/head/pwd),再考慮 bounded general exec。每工具威力被 binding 界定;deny-by-default/credential-blind/commit-before-effect/WORM。spec:`HDI2-governed-toolset.md` | ✅ **HDI2a DONE**(5 read-only 工具 exec.cat/head/pwd/wc/grep:manifest+binding 純加法,argv 純 vector 永不 sh -c〔grep `-e` 防 pattern-as-flag〕,strict string-only schema;tools/list 自動含 7 工具〔schema 衍生〕,exact-set 斷言 across 4 檔更新+bite;無治理放寬,mutation 證實;16 單元測;EXEC4a/4c/pipeline 未動;獨立 Opus4.8 review PASS)。✅ **INFO polish DONE**(MCP tools/list description 從 registry manifest 取,no-drift)。✅ **HDI2b DONE**(bounded general `exec.run {argv}`:argvPrefix []、strict string-array schema、toArgv [...argv] **永不 sh -c**;argSchemaToJsonSchema 擴充 string-array〔其餘 fail-closed〕;exec.run 走同一治理唯一 edge,unregistered/smuggled/secret 皆 deny substrate 0;exact-set→8;verify 1081;獨立 Opus4.8 review PASS;邊界=binding+sealed sandbox,user-approved posture)。**剩:新工具的 live drive(選,user-initiated)** |
