...
| **EXEC3b**(capstone,live,**user-initiated**) | 真 Hermes × 真 OpenShell:**(A)** 對真 brain 證 SAFETY(它提案它自己的工具→deny-by-default/propose-only/credential-blind/bounded/ephemeral-sandbox)+ **(B)** success 路徑(受控 exec.echo/ls 提案 over 真 OpenShell→真輸出回饋)+ 真 inbound screen(finding②)。dual-gate。spec:`EXEC3b-live-capstone.md`。**誠實:真 Hermes 自主呼叫我們的工具需 MCP 廣告=EXEC4;真 credential 邊界=sandbox 零憑證+無 egress** | DRAFT(spec 好,待開工)|
