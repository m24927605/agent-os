...
| **EXEC3b**(capstone,live,**user-initiated**) | 真 OpenShell ephemeral sandbox + 真 desktop Hermes ACP,小 maxTurns/緊 cost:propose→govern→真命令→真輸出→續推。**真正的 credential 邊界 = sandbox 零真憑證 + 無 egress(非 redaction)** | OPEN(待 EXEC3a + 你親跑)|
