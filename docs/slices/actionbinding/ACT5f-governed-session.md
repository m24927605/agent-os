...
- [ ] RED → verify exit 0(BrowserStep += session.open/close + Fake/contract perform〔mint/release〕+ browser.session.open〔strict {}, write, 回 minted sessionId〕/close〔{sessionId}〕manifest+binding + session cap fail-closed + seedBrowserRegistry/Bindings 註冊 → ACT5e advertise 自動涵蓋;mint+用、unknown-session deny、moat〔只回 opaque id〕、cap fail-closed、end-to-end brain-driven governed 流程;ACT5e session-advertise 斷言更新;mutation 證;byte-identical 其餘;depcruise/secret-scan clean;無新依賴);獨立 Opus 4.8 review PASS。
