...
| Slice | 範圍 | infra | 狀態 |
|---|---|---|---|
| **OS-S1** | mTLS gRPC channel(讀 ca/tls cert 連 17670)+ pin 真實 proto 子集(擴 `openshell.subset.proto` 加 `ExecSandbox`+messages)+ regen codec + **`ExecSandbox` server-streaming** → 實作 `OpenShellExecTransport`;單元測試(proto codec + fake stream)+ **live 驗證**(對真 gateway exec)| 真 gateway(已就緒)| DRAFT |
| **OS-S2** | `CreateSandbox`/`GetSandbox`/`WatchSandbox`(stream)/`DeleteSandbox` → 完成 `OpenShellLifecycleTransport` + `OpenShellReadinessTransport`;live 驗證(create→ready→delete)| 真 gateway | DRAFT |
| **NC-S11b** | NemoClaw composition root 綁定(`ExecLike`→`execSandbox`:ctx-closure + `["/bin/sh","-c",cmd]` 包裝)+ gated live e2e(host trivial gateway〔python3 /health on 18789〕於真 sandbox → status running → reconcile)+ harness `e2e-live-nemoclaw.sh`(preflight openshell;create+destroy sandbox 用 CLI)| 真 gateway + sandbox | DRAFT |
