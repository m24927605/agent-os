...
| **SETUP2**(`agentos setup` wizard + `agent-os.config.yaml`) | 宣告式 config(固定 schema,zero-dep subset reader)→ 驗證(fail-closed)→ 註冊 bin 進 Hermes(互動委派 `hermes mcp add` / `--print` headless,非破壞)→ 跑 doctor。readline 補問。SpendGuard turnkey;AGT 待 endpoint adapter。spec:`SETUP2-wizard.md` | DRAFT(spec 好,待核准) |
