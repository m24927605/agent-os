# SLICE-SETUP2: `agentos setup` wizard + 宣告式 `agent-os.config.json`
...
## (0) 動機
SETUP1b 的 `agentos doctor` **驗證**前置;SETUP2 **產生 + 套用**——讀一份宣告式 `agent-os.config.json`(契合你「先提供 yaml」的直覺),驗證(fail-closed),把 Agent OS 的 governed MCP bin 註冊進 Hermes,再跑 doctor。**不做 TUI**(thin/zero-dep CLI + 宣告式 config 是定案)。

## (1) `agent-os.config.json`(固定 schema;**JSON**,`JSON.parse` 零依賴、最穩)
**決定:用 JSON**(非 yaml)——`JSON.parse` 零依賴且不脆(避免 in-house YAML parser 風險)。
```json
{
  "openshell": { "endpoint": "127.0.0.1:17670", "mtlsDir": "~/.config/openshell/gateways/openshell/mtls", "image": "ghcr.io/.../openclaw@sha256:..." },
  "kernel": { "ingestEndpoint": "127.0.0.1:50051" },
  "spendguard": { "udsPath": "/path/to/sidecar.sock", "budgetId": "...", "unitId": "...", "windowInstanceId": "..." }
}
```
- `openshell`/`kernel` 必要;`spendguard` 可選(設了即 gate;SETUP1a 已讓 bin 接);`agt` 見 open decision(follow-up)。
- 全為**非-secret**(端點/路徑/id);**絕不放金鑰**。讀 = `JSON.parse` + **zod `.strict()` schema 驗證**(unknown key/型別錯/缺必填 → fail-closed throw,不猜)。

## (2) `agentos setup [--config <path>] [--print] [--non-interactive]`
1. **Load**:讀 `agent-os.config.json`(預設 `./agent-os.config.json` 或 `--config`)→ JSON.parse + zod 驗證。**互動模式**:缺欄位用 Node 內建 `readline` 補問(零依賴);`--non-interactive` 則缺欄位即 fail。
...
## (4) 不變量
fail-closed 驗證(部分/malformed config → 非零、不寫半套)/ credential-blind(config/env 只非-secret;readline 不回顯敏感、輸出不印值;**絕不寫/印金鑰**)/ 非破壞(Hermes-owned merge 或 print,不脆性 auto-merge)/ zero-dep(in-house JSON.parse + zod 驗證 + readline)/ idempotent(`hermes mcp add` upsert)。

## (5) Test-first plan（RED 先行)
- JSON.parse + zod 驗證:valid config → parsed 物件;malformed/非預期 → fail-closed throw。
