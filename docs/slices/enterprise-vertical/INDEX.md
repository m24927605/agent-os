...
| **ES2a** | live per-tenant WORM 基礎(contract+Go):`proto/ingest.proto` 加 `partition_id` + kernel server `Append` 依 partition_id 路由到既有 `PartitionedIngest`(per-tenant Store/Signer)+ Go conformance(per-tenant 鏈/key 獨立、空/未知 partition_id deny)| DRAFT(先建;Go+proto)|
