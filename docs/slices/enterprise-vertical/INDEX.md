...
| **ES2a** | live per-tenant WORM 基礎(contract+Go):`proto/ingest.proto` 加 `partition_id` + kernel server `Append` 依 partition_id 路由到既有 `PartitionedIngest`(per-tenant Store/Signer)+ Go conformance(per-tenant 鏈/key 獨立、空/未知 partition_id deny)| ✅ **DONE**(drift gate + fail-closed + head-independence mutation 證實;單鏈 byte-identical;獨立 Opus4.8 review PASS)|
| **ES2b** | Enterprise per-tenant live WORM(消費者):TS transport 送 partition_id + per-tenant live wormSink + `EnterpriseFleetOpts.wormSinkFor`(default in-memory,byte-identical)+ gated live e2e(Enterprise 寫面接真實 kernel partition、kernel 層跨租戶隔離)| ✅ **DONE + LIVE 1/1**(field-4 encode + wormSinkFor mutation 證實;default byte-identical;A 的 marker 不入 B 的 WAL;2 live-bug 修;獨立 Opus4.8 review PASS)|
...
| **ES4** | 租戶生命週期:動態 `registerTenant`/`deprovisionTenant`(immutable-rebuild router + store 附加 register + per-tenant deps provision;deprovision=撤路由,WORM append-only 不抹除)| ✅ **DONE**(動態註冊隔離 3 mutation 證實;deprovision fail-closed;re-register→clean denied;static ctor byte-identical;獨立 Opus4.8 review PASS;動態 in-memory,live kernel partition=P4)|
