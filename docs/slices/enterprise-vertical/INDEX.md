...
| **ES4** | 租戶生命週期:動態 `registerTenant`/`deprovisionTenant`(immutable-rebuild router + store 附加 register + per-tenant deps provision;deprovision=撤路由,WORM append-only 不抹除)| DRAFT(spec 好,待開工;動態 in-memory,live kernel partition=P4)|
