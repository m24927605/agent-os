...
| **ES2b** | Enterprise per-tenant live WORM(消費者):TS transport 送 partition_id + per-tenant live wormSink + `EnterpriseFleetOpts.wormSinkFor`(default in-memory,byte-identical)+ gated live e2e(Enterprise 寫面接真實 kernel partition、kernel 層跨租戶隔離)| DRAFT(依 ES2a)|
