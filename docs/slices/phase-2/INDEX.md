...
> **狀態（2026-06-21）：P2-B/A/C/D/E/F/G/H 已 doc-first 實作並 merge（每個皆 RED→GREEN→fresh IV PASS→--no-ff merge）。**
...
## 1. Slice 清單與 DAG（實作順序 B→A→C→D→E→F→G→H）
...
```
> 無 cycle 證明：rank = 0（B,C,E,F,G,H）/ 1（A）/ 2（D）；每條邊嚴格遞減 ⇒ DAG。
