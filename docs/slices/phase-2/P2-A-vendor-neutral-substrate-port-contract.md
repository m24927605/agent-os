...
> 預期首次 RED：import `../runtime/substrate/index.js` 失敗（模組不存在）→ 升級為斷言失敗（先建 stub）。

## (6) Definition of Done（待填）
- [ ] first RED exit code 已貼。
- [ ] `pnpm run verify` exit 0。
- [ ] 舊 `runtime/openshell/` 已刪、無懸空 import；`no-vendor-in-core`（P2-B）仍綠、substrate 路徑零 vendor token。
- [ ] secret-scan clean。
- [ ] Adversarial review = PASS（含 mutation：4 種壞 adapter — throws / status≠event / 允許壞 ctx / ok 無 id — 皆被 contract 抓）。
