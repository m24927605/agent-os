...
- [x] **first RED**（pipeline 不存在）：`vitest run pipeline.e2e.test.ts` → import 失敗、no tests（exit≠0）。
- [x] `pnpm run verify` **exit 0**（137 tests、deps 33 modules 0 violations、secret-scan clean）。
- [x] `deps:check` 綠（IV 確認 pipeline.ts 只 import `../commitgate/index.js` + `../cost/index.js`（深度 1 barrel，豁免）、對 toolcall 形狀泛型化（不 import brain，避開 not-to-internal）、無 vendor、無 cycle）。
- [x] secret-scan clean（secret canary runtime 組裝）。
- [x] **Adversarial review = PASS**（fresh-context IV，零 defect；effect 在 screen/policy/cost/commit 短路時**皆 0 次**、happy path **恰 1 次**、first-gate-wins、commit-abort 不 cost.commit；4 種 mutation（effect 移到任一 gate 前 / commit-abort 仍跑）皆被抓）。採納 IV follow-up #2：把「effect 恰一次 / 短路 0 次」的 invocation-count spy 永久折進 e2e 套件（防 double-run 回歸）。
> 已知 follow-up（§8）：commit-abort 後 reserve 仍 held（CostGate 無 release op）——後續 slice 處理。
